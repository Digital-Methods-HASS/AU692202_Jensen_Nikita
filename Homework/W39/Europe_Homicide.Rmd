...
date: " created 3 October 2023, updated `r format(Sys.time(), '%d %B %Y')`"
output: 
  html_document:
    toc: true
    toc_float: true
...
Found the data and put it in my data folder in R. Allows me to read the data below.
```{r load-data}
...
Note to self - remember to make eval = true in the code space to get the plots in the knitted html version.

```{r plot-homicides, eval = TRUE}
ggplot(data = Western_Europe, aes(x = Year, y = homicides_per_100k, color = Entity))+
  geom_line()+
  labs(title = "Western Europe Homicide Trends", 
       y = "Homicides p 100k")
...
**Nikita's solution:**
```{r eval = TRUE}
DanishMonarchs <- read_csv2("data/Kings.csv")

DanishMonarchs %>% 
  select(Name, End_Reign, Start_Reign) %>% 
  mutate(duration_of_reign = End_Reign - Start_Reign) %>% 
  mutate(midyear = End_Reign - (End_Reign - Start_Reign)/2) %>% 
  ggplot(aes(x = midyear, y = duration_of_reign))+
  geom_smooth()+
  labs(title = "Duration Danish Monarchs", 
       x = "Years", 
       y = "Duration of reign")
```
...
**Nikita's answer:**
There is a rise in the durationtime for the danish monarchs in the late middle ages until around 1700. An option is that societal changes can have had an impact on this. Danish monarchs were given the opportunity to reign longer because of these changes. 
...
I edited the YAML Header (the top in an RMarkdown) to include a floating table and a timestamp (last updated).
...
The visualization shows that European Countries have become less violent over time. The duration over Danish monarchs shows that mornarchs reign over a longer period of time. This can be because of societal changes, which also can be the reason for the declining of homicides in the European Countries. Societal changes can have an impact on the stability. So based on this data and visualization I would say that we are more civilized today because of the decrease in killings.